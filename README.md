...
### Clipping
Choose **cranial**, **facial**, or **manual** clipping. The clip boundary is left open, not capped, since repair runs before clipping, so exported meshes may appear open at the cut.
...
### Mesh cleanup
After registration: repair (always on, since a real scan can be thousands of disconnected pieces and repair is what makes it one usable surface), clip, then resample to a target vertex count (optional, default 10000). The measurement algorithm was validated at 10000 vertices.
